...
The generator creates a focused App Router-style starter: `app/pages.tsx`, `app/globals.css`, automatic minimize and auth UI, Home and Me embeds, an authenticated API route, and an audience WebSocket example. The live Home dashboard counts unique users connected globally, viewing Home, and sharing the current Discord channel. Framework defaults handle optional generic loading, error, metadata, layout, and not-found conventions until an app chooses to add them. The starter also includes strict TypeScript and React configuration, environment templates with configurable local Discord identities, mobile safe-area CSS, graceful shutdown settings, same-origin request protection, and safe API/WebSocket defaults. It refuses to overwrite a non-empty target directory.
