...
- **Unified file-system routing:** put `page.tsx`, `route.ts`, `socket.ts`, and `embed.tsx` anywhere under `app/`. The pathname chooses the nearest page; the hash chooses an embed owned by that page. No literal `prefix/` directory is required.
- **Simple by default:** start with one `app/page.tsx` and local embeds. Add more pathname pages only when project scale or an independent surface needs its own URL, shell, or provider lifetime.
- **Fast Refresh and recovery:** page, embed, component, and CSS edits preserve compatible React state. Structural edits regenerate routes, while initial errors keep the development server and diagnostics page alive.
- **Safer Discord requests:** same-origin traffic and only the configured application's exact `https://<clientId>.discordsays.com` origin are accepted by default.
- **Version-matched AI guidance:** npm ships root `AGENTS.md`, detailed guides under `dist/docs/`, and routed skills under `dist/skills/`.
