...
- `server.allowedOrigins` now defaults to `"discord-activity"` inside the framework instead of being repeated in generated projects.
- HTTP routes and WebSocket upgrades accept same-origin traffic plus this application's exact `https://<clientId>.discordsays.com` Activity proxy origin.
- The proxy origin is derived from `discord.clientId` or `LUDICORD_DISCORD_CLIENT_ID`; other applications on `discordsays.com` remain rejected.
- Use `"same-origin"` for a host-only policy. Arrays must include both `"same-origin"` and `"discord-activity"` when retaining both built-in policies alongside another trusted browser origin.
